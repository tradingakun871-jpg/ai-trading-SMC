...
#property description "AI Trading SMC V4 Scalping Real-Time Feed - data only"
...
string TFName(ENUM_TIMEFRAMES tf){if(tf==PERIOD_M1)return "M1";if(tf==PERIOD_M3)return "M3";if(tf==PERIOD_M5)return "M5";return "UNKNOWN";}
...
void SendInitialHistory(){SendHistory(PERIOD_M1);SendHistory(PERIOD_M3);SendHistory(PERIOD_M5);}
void SendRealtime(){SendLive(PERIOD_M1);SendLive(PERIOD_M3);SendLive(PERIOD_M5);}
int OnInit(){if(LiveIntervalSeconds<1||HistoryBars<20)return INIT_PARAMETERS_INCORRECT;EventSetTimer(LiveIntervalSeconds);Print("AI Trading SMC V4 Scalping started. M5 bias -> M3 confirm -> M1 entry. Interval=",LiveIntervalSeconds," sec. Auto trade OFF.");SendInitialHistory();SendRealtime();return INIT_SUCCEEDED;}
